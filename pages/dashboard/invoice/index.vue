...
const allColumnsSelected = computed(() => {
  return availableColumns.value.every((col) =>
    selectedColumns.value.has(col.key)
  );
});
...
async function getData() {
  ...
  try {
    ...
    data.forEach((invoice: any, idx: number) => {
      ...
      const code =
        invoice?.customer?.area?.code_name ||
        invoice?.customer?.area?.codeName ||
        "";
    ...
    console.log(
      "Invoice data updated in customer.value:",
      ...
      if (newStatus === "paid") {
        ...
        setTimeout(async () => {
          try {
            ...
            notification.error(
              "Error",
              'Gagal membuka PDF otomatis. Silakan klik "Download PDF" secara manual.',
              5000
            );
          }
        }, 1000);
...
function cancelStatusChange() {
  if (statusConfirmationData.value) {
    ...
    const invoiceIndex = customer.value.findIndex(
      (inv) => inv.id === statusConfirmationData.value!.invoiceId
    );

    if (invoiceIndex !== -1) {
      customer.value[invoiceIndex].status =
        statusConfirmationData.value.currentStatus;
    }
  }
...
async function handlePdfView(invoiceId: string, isAutoOpen: boolean = false) {
  ...
  try {
    ...
    if (invoice?.pdf_viewed) {
      ...
      notification.error(
        "PDF Sudah Dilihat",
        "PDF invoice ini sudah pernah dilihat dan tidak dapat diakses lagi untuk mencegah duplikasi pembayaran.",
        5000
      );
    ...
    try {
      ...
      notification.warning(
        "Warning",
        "Gagal menandai PDF sebagai dilihat, tetapi PDF tetap akan dibuka.",
        3000
      );
    }
    ...
    if (isAutoOpen) {
      notification.success(
        ...
        "PDF invoice dibuka otomatis. PDF ini tidak dapat dibuka lagi untuk mencegah duplikasi pembayaran.",
        5000
      );
    } else {
      notification.info(
        "PDF Dibuka",
        "PDF invoice telah dibuka. PDF ini tidak dapat dibuka lagi untuk mencegah duplikasi pembayaran.",
        5000
      );
    }
  } catch (error: any) {
    ...
    if (error.message && error.message.includes("Unexpected token")) {
      console.error(
        "JSON parsing error - server returned HTML instead of JSON"
      );

      notification.error(
        "Error",
        "Server error: PDF tidak dapat dibuka. Silakan coba lagi.",
        5000
      );
    } else {
...
async function createRecurringFromInvoice() {
  ...
  const base = await invoiceAdminApi().getInvoice(
    selectedInvoiceForRecurring.value.id
  );
  ...
  try {
    await recurringInvoiceAdminApi().createRecurringInvoice({
      ...
      invoice_date: new Date(
        recurringForm.invoice_date + "T00:00:00.000Z"
      ).toISOString(),

      due_date: new Date(
        recurringForm.due_date + "T00:00:00.000Z"
      ).toISOString(),
    ...
    notification.error(
      "Failed to start recurring",
      err?.message || "Failed to start recurring"
    );
  }
}
...
function getRouterJobState(invoiceId: string): {
  state: "none" | "pending" | "error" | "success";
  message?: string;
} {
  ...
  const anyError = jobs.some(
    (j: any) => (j.status || "").toLowerCase() === "error"
  );
  ...
  if (anyError) {
    ...
    return {
      state: "error",
      message: err?.last_error || "Router update failed",
    };
  }

  const anyPending = jobs.some(
    (j: any) => (j.status || "").toLowerCase() === "pending"
  );
...
const filteredRows = computed(() => {
  ...
  if (dateFilter.value) {
    filteredData = filteredData.filter((invoice) => {
      ...
      return (
        (invoiceDate &&
          invoiceDate.toDateString() === filterDate.toDateString()) ||
...
function generateThermalDataForInvoices(invoices: any[]): string {
  ...
  for (let i = 0; i < invoices.length; i++) {
    ...
    if (selectedColumns.value.has("customer_display")) {
      output +=
        centerText(invoice.customer?.name || "Unknown Customer", printerWidth) +
        "\n";
    }
    ...
    if (selectedColumns.value.has("number")) {
      output +=
        formatKV("Nomor Tanda", invoice.number || invoice.id, printerWidth) +
        "\n";
      output +=
        formatKV("Terima", invoice.number || invoice.id, printerWidth) + "\n";
    }
    ...
    if (
      selectedColumns.value.has("amount") ||
      selectedColumns.value.has("amount_due") ||
      selectedColumns.value.has("total_paid")
    ) {
      ...
      if (invoice.invoice_items && invoice.invoice_items.length > 0) {
        for (let j = 0; j < invoice.invoice_items.length; j++) {
          ...
          serviceName = serviceName.replace(
            /\s+[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}.*$/,
            ""
          );
          // Also remove any remaining MAC-like patterns
          serviceName = serviceName.replace(
            /\s+[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}.*$/,
            ""
          );
        ...
        output += `Internet Service - ${new Date().toLocaleDateString("en-US", {
          month: "short",
        })} - Rp ${fallbackPrice}\n`;
      }
    ...
    if (selectedColumns.value.has("amount_due")) {
      ...
      output +=
        formatKV("Saldo", "Rp " + formatCurrency(amountDue), printerWidth) +
        "\n";
    ...
    if (selectedColumns.value.has("status")) {
      output +=
        formatKV("Status", invoice.status?.toUpperCase() || "-", printerWidth) +
        "\n";
    }
...
async function printAllUnpaidInvoices() {
  ...
  try {
    ...
    const unpaidPendingInvoices = filteredInvoices.filter(
      (invoice) =>
        invoice.status?.toLowerCase() === "unpaid" ||
        invoice.status?.toLowerCase() === "pending"
    );
    ...
    if (unpaidPendingInvoices.length === 0) {
      notification.warning(
        "No Invoices",
        "No unpaid or pending invoices found in the current filter."
      );
      return;
    }
    ...
    toast.add({
      ...
      description: `Generated report for ${
        unpaidPendingInvoices.length
      } invoices (${
        statusFilter.value || "all statuses"
      }). Use the controls in the new window to print or save as PDF.`,
      color: "green",
    });
  } catch (error: any) {
...
<template>
  ...
  <div
    class="bg-gray-50 p-4 rounded-lg border border-gray-200 dark:border-gray-700 mb-4"
  >
  ...
  <UTable :rows="filteredRows || []" :columns="columns || []" :loading="isLoading">
    ...
    <template #amount_due-data="{ row }">
      ...
      >
        {{ currency.formatIDR(getAmountDue(row)) }}
      </p>
    </template>
    ...
    <template #status-data="{ row }">
      ...
      <div
        class="mb-1"
        @mouseenter="ensureJobsLoaded(row.id)"
        @touchstart.passive="ensureJobsLoaded(row.id)"
      >
        <template v-if="getRouterJobState(row.id).state === 'pending'">
          <span
            class="inline-flex items-center px-2 py-0.5 text-[10px] font-medium rounded-full bg-blue-100 text-blue-700"
          >
        ...
        <template v-else-if="getRouterJobState(row.id).state === 'error'">
          ...
          >
            <span
              class="inline-flex items-center px-2 py-0.5 text-[10px] font-medium rounded-full bg-red-100 text-red-700"
            >
            ...
            <UButton
              ...
              color="red"
              variant="outline"
              @click.stop="retryRouterJobs(row.id)"
              >Retry</UButton
            >
          </div>
        </template>
        ...
        <template v-else-if="getRouterJobState(row.id).state === 'success'">
          <span
            class="inline-flex items-center px-2 py-0.5 text-[10px] font-medium rounded-full bg-emerald-100 text-emerald-700"
          >
            Router updated
          </span>
        </template>
      ...
      <div class="space-y-2">
        ...
        <div v-if="row.status === 'pending'" class="space-y-2">
          ...
          <div
            v-if="row.pending_reason"
            class="mt-2 p-2 bg-yellow-50 dark:bg-black/70 border border-yellow-200 dark:border-yellow-700 rounded-lg"
          >
            <div class="flex items-start gap-2">
              <svg
                class="w-4 h-4 text-yellow-200 dark:text-yellow-400 flex-shrink-0 mt-0.5"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
              ...
              <div class="flex-1">
                <p
                  class="text-xs font-medium text-yellow-800 dark:text-yellow-200 mb-1"
                >
                  Pending Reason:
                </p>

                <p
                  class="text-xs text-yellow-700 dark:text-yellow-300 leading-relaxed"
                >
                  {{ row.pending_reason }}
                </p>
              </div>
        ...
        <div v-else-if="row.status === 'paid'">
          ...
          <span
            class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800 cursor-not-allowed w-full justify-center"
          >
        ...
        <div
          v-if="isPdfViewed(row.id)"
          class="flex items-center text-xs text-red-600 justify-center"
        >
          <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
            <path
              fill-rule="evenodd"
    ...
    >
        ...
        <div v-if="statusConfirmationData" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p class="font-medium text-gray-900 dark:text-white">
                  {{
                    statusConfirmationData.invoiceData?.customer?.name || "N/A"
                  }}
                </p>
              </div>
              ...
              <div>
                ...
                <p class="font-medium text-gray-900 dark:text-white">
                  {{
                    currency.formatIDR(
                      statusConfirmationData.invoiceData?.amount || 0
                    )
                  }}
                </p>
              </div>
                ...
                <span
                  ...
                  :class="{
                    'bg-red-100 text-red-800':
                      statusConfirmationData.currentStatus === 'unpaid',

                    'bg-yellow-100 text-yellow-800':
                      statusConfirmationData.currentStatus === 'pending',

                    'bg-green-100 text-green-800':
                      statusConfirmationData.currentStatus === 'paid',
                  }"
                >
            ...
            <div class="flex">
              ...
              <div class="ml-3">
                ...
                <div class="mt-2 text-sm text-yellow-700 dark:text-yellow-300">
                  <p>
                    Setelah status diubah menjadi <strong>PAID</strong>, status
                    ini tidak dapat diubah kembali. Pastikan pembayaran sudah
                    benar-benar diterima sebelum melanjutkan.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="text-center">
            <p class="text-lg font-medium text-gray-900 dark:text-white">
              Apakah Anda yakin ingin mengubah status pembayaran menjadi
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          <UButton color="gray" variant="soft" @click="cancelStatusChange">
            Batal
          </UButton>
    ...
    >
      ...
      <template #footer>
        <div class="flex justify-end gap-2">
          <UButton color="gray" variant="soft" @click="closeStartRecurringModal"
            >Cancel</UButton
          >

          <UButton color="green" @click="createRecurringFromInvoice"
            >Start</UButton
          >
        </div>
      </template>
    ...
    >
      <template #header>
        <div class="flex items-center justify-between">
          ...
          <UButton
            :icon="
              allColumnsSelected ? 'i-heroicons-x-mark' : 'i-heroicons-check'
            "
            color="gray"
            variant="ghost"
            size="sm"
