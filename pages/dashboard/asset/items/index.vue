...
const filteredRows = computed(() => {
  if (!q.value && !selectedAssetId.value) {
    return defaultAssetItems.value.slice(
  ...
  if (selectedAssetId.value) {
    filteredData = filteredData.filter(
      (item: any) => item.asset_id === selectedAssetId.value
    );
  }
...
if (route.query.edit) {
  ...
  const itemToEdit = defaultAssetItems.value.find(
    (item: any) => item.id === route.query.edit
  );
  if (itemToEdit) {
...
<template>
  <div class="space-y-4">
    ...
    <div
      class="flex gap-4 px-3 py-3.5 border-b border-gray-200 dark:border-gray-700"
    >
    ...
    <div class="table-scroll-container">
      ...
      <div class="table-scroll-footer">
        <span class="scroll-hint"
          >↔ Scroll horizontally to see more columns | ↕ Scroll vertically for
          more rows</span
        >
      </div>
    </div>
