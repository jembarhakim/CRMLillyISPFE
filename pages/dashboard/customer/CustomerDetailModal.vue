    ...
    <div class="bg-white rounded-lg shadow-xl w-full max-w-6xl max-h-[95vh] overflow-hidden flex flex-col">
      ...
      <div v-else-if="customerDetail" class="flex flex-col lg:flex-row flex-1 overflow-hidden">
        ...
        <div v-if="activeTab === 'summary'" class="space-y-6">
          ...
          <div class="bg-white border border-gray-200 rounded-lg p-4 sm:p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p class="mt-1 text-sm text-gray-900">{{ formatDate(customerDetail.customer.service_request_date) || 'N/A' }}</p>
              </div>
              ...
              <!-- NEW: Proposed Package from Add Customer form -->
              <div>
                <label class="block text-sm font-medium text-gray-700">Paket yg Diajukan</label>
                <p class="mt-1 text-sm text-gray-900">{{ getProposedPackageNameEnhanced(customerDetail.customer.proposed_package) }}</p>
              </div>
              ...
              <div>
                ...
                <p class="mt-1 text-sm text-gray-900">{{ getSalesRepresentativeName(customerDetail.customer.sales_representative_id) }}</p>
              </div>
              ...
              <div>
                ...
                <p class="mt-1 text-sm text-gray-900">{{ formatDate(customerDetail.customer.installation_date) || 'Belum diinstal' }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-green-50 rounded-lg p-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p class="mt-1 text-sm text-gray-900">{{ formatDate(customerDetail.customer.installation_date) || 'N/A' }}</p>
              </div>
              
              <!-- Updated: Next Payment Date -->
              <div>
                <label class="block text-sm font-medium text-gray-700">Next Payment Date</label>
                <p class="mt-1 text-sm text-gray-900">{{ formatDate(customerDetail.customer.next_payment_date) || 'N/A' }}</p>
              </div>
            </div>
          </div>
          ...
          <div v-if="customerDetail.installations && customerDetail.installations.length > 0 && customerDetail.network_devices && customerDetail.network_devices.length > 0" class="bg-purple-50 rounded-lg p-4">
              ...
              >
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <p class="text-sm text-gray-900">{{ device.assets ? `${device.assets.brand} ${device.assets.model}` : 'N/A' }}</p>
                  </div>
                  ...
                  <div>
                    <label class="block text-xs font-medium text-gray-700">Status Perangkat</label>
                    <span 
                      :class="[
                        'inline-flex px-2 py-1 text-xs font-medium rounded-full',
                        device.status_perangkat === 'active' ? 'bg-green-100 text-green-800' : 
                        device.status_perangkat === 'inactive' ? 'bg-red-100 text-red-800' : 
                        device.status_perangkat === 'maintenance' ? 'bg-yellow-100 text-yellow-800' : 
                        device.status_perangkat === 'faulty' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'
                      ]"
                    >
                      {{ device.status_perangkat?.toUpperCase() || 'N/A' }}
                    </span>
                  </div>
                  ...
                  <div>
                    ...
                    <div class="flex items-center space-x-2">
                      ...
                      >
                        {{ device.last_ping_status?.toUpperCase() || 'N/A' }}
                      </span>
          ...
          <div v-if="customerDetail.installations && customerDetail.installations.length > 0" class="bg-yellow-50 rounded-lg p-4">
            <div class="flex justify-between items-center mb-4">
              <h3 class="text-lg font-medium text-gray-900">Installation Reports ({{ customerDetail.installations.length }})</h3>
              <UButton 
                color="green" 
                size="sm"
                @click="addNewInstallationReport"
              >
            ...
            <div class="space-y-3">
              <div 
                v-for="(installation, index) in customerDetail.installations" 
                :key="installation.id"
              ...
              >
                <div class="flex justify-between items-start mb-3">
                  <h4 class="text-sm font-semibold text-gray-900">
                    Report #{{ index + 1 }} 
                    <span v-if="index === 0" class="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full ml-2">Latest</span>
                  </h4>
                  <span class="text-xs text-gray-500">{{ formatDate(installation.createdAt) }}</span>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p class="text-sm text-gray-900">{{ formatDate(installation.installation_completed_at) || 'N/A' }}</p>
                  </div>
                  
                  <!-- NEW: Installation Team Name from installation report -->
                  <div>
                    <label class="block text-xs font-medium text-gray-700">Installation Team</label>
                    <p class="text-sm text-gray-900">{{ installation.installation_team_name || 'N/A' }}</p>
                  </div>
                  
                  <!-- NEW: Installation Team Phone from installation report -->
                  <div>
                    <label class="block text-xs font-medium text-gray-700">Team Phone</label>
                    <p class="text-sm text-gray-900">{{ installation.installation_team_phone || 'N/A' }}</p>
                  </div>
                  
                  <!-- Updated: Date (renamed to Installation Date) -->
                  <div>
                    <label class="block text-xs font-medium text-gray-700">Installation Date</label>
                    <p class="text-sm text-gray-900">{{ formatDate(installation.date) || 'N/A' }}</p>
                  </div>
                  
                  <!-- Updated: Description (renamed to Notes) -->
                  <div class="md:col-span-2">
                    <label class="block text-xs font-medium text-gray-700">Notes</label>
                    <p class="text-sm text-gray-900">{{ installation.notes || installation.description || 'No notes' }}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-else class="bg-yellow-50 rounded-lg p-4">
            <div class="flex justify-between items-center">
              <div>
                ...
                <p class="text-xs text-gray-500 mt-1">Installation Type, Status, On Air Date, and Service Ready Date will be available after creating an installation report.</p>
              </div>
              <UButton 
                color="green" 
                size="sm"
        ...
        <div v-if="activeTab === 'connection'" class="space-y-6">
          ...
          <div v-if="customerDetail.installations && customerDetail.installations.length > 0 && customerDetail.network_devices && customerDetail.network_devices.length > 0" class="bg-white border border-gray-200 rounded-lg p-6">
          ...
          <div v-else-if="!customerDetail.installations || customerDetail.installations.length === 0" class="bg-white border border-gray-200 rounded-lg p-6">
            <div class="text-center">
              ...
              <p class="text-gray-600 mb-4">Network devices will appear after creating an installation report for this customer.</p>
              <UButton 
                color="blue" 
                size="sm"
                @click="addNewInstallationReport"
          ...
          <div v-else class="bg-white border border-gray-200 rounded-lg p-6">
            <div class="text-center">
              ...
              <p class="text-gray-600">This customer has installation reports but no network devices configured yet.</p>
            </div>
          </div>
        </div>
...
const fetchInstallationReports = async (customerId: string) => {
  try {
    ...
    if (response.success && response.data) {
      ...
      return { success: true, data: customerReports, message: 'Success' }
    }
      ...
      if (customerDetail.value) {
        ...
        if (customerInstallations.length > 0) {
          ...
          customerDetail.value.customer = {
            ...
            installation_team_name: latestInstallation.technician_name,
